...
public class ParallelSimulation{
    ...
    */
	...
	private int threshold_forkjoin_getavoid(int n_states){
		return Math.max(10,n_states/16);
	}
	private int threshold_forkjoin_Fair_refine(int n_states){
		return Math.max(10,n_states/16);
	}

	private int threshold_jumping_getavoid(int n_states){
		return Math.max(10,n_states/16);
	}
	private int threshold_jumping_refine_W(int n_states){
		return Math.max(10,n_states/16);
	}
...
*/
	...
	public Set<Pair<FAState,FAState>> JumpingDirectSimRelNBW(FiniteAutomaton omega1, int la)
	{
		...
		// Compute transitive closure
		close_transitive(W,n_states);
		// System.out.println("JD: Gotten BLA forward");
		get_jump(jump, jump_len, acc_jump, acc_jump_len, W, isFinal, n_states);
...
// --------------------------------------------------------------------------------------------------
	...
	public boolean JumpingBLAFairSimRelNBW(FiniteAutomaton omega1, FiniteAutomaton omega2, int la, int bwchoice)
	{
		...
		while(changed){
			...
			changed=(forkJoinPool.invoke(new par_JumpingBLAFair_refine_W(n1,n2,0,n1,0,n2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,la)) >0);
...
//		boolean result1 = IntStream.range(0, jump_len[q])
//				.parallel()
//				.anyMatch(r -> post_len2[s][jump[q][r]] > 0);
//
//		boolean result2 = IntStream.range(0, acc_jump_len[q])
//				.parallel()
//				.anyMatch(r -> post_len2[s][acc_jump[q][r]] > 0);
//
//		return result1 || result2;
	}

	//NOTICE:
	class par_JumpingBLAFair_refine_W extends RecursiveTask<Integer> {
		int p1,p2,q1,q2;
		int n1, n2;
		int n_symbols;
		int[][][] post1;
		...
		par_JumpingBLAFair_refine_W(int n1, int n2, int p1, int p2, int q1, int q2, int n_symbols, int[][][] post1, int[][] post_len1, int[][][] post2, int[][] post_len2, int[][] jump, int[] jump_len, int[][] acc_jump, int[] acc_jump_len, boolean[][] W, int la){
		...
		protected Integer compute(){
			...
			if ((p2-p1 <= threshold_jumping_refine_W(n1)) || (q2-q1 <= threshold_jumping_refine_W(n2))) {
				return single_JumpingBLAFair_refine_W(p1,p2,q1,q2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,la);
			}
			else{
				par_JumpingBLAFair_refine_W t1 = new par_JumpingBLAFair_refine_W(n1,n2, p1,p1+(p2-p1)/2,q1,q1+(q2-q1)/2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,la);
				par_JumpingBLAFair_refine_W t2 = new par_JumpingBLAFair_refine_W(n1,n2,p1,p1+(p2-p1)/2,q1+(q2-q1)/2,q2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,la);
				par_JumpingBLAFair_refine_W t3 = new par_JumpingBLAFair_refine_W(n1,n2,p1+(p2-p1)/2,p2,q1,q1+(q2-q1)/2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,la);
				par_JumpingBLAFair_refine_W t4 = new par_JumpingBLAFair_refine_W(n1,n2,p1+(p2-p1)/2,p2,q1+(q2-q1)/2,q2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,la);
				t2.fork();
				t3.fork();
				t4.fork();
				int r1=t1.compute();
	...
	private int single_JumpingBLAFair_refine_W(int p1, int p2, int q1, int q2, int n_symbols, int[][][] post1, int[][] post_len1, int[][][] post2, int[][] post_len2, int[][] jump, int[] jump_len, int[][] acc_jump, int[] acc_jump_len, boolean[][] W, int la)
	{
		//TODO:
		int[] attack = new int[2*la+1];
		boolean changed=false;
	...
	//NOTICE:
	class par_JumpingBLAFair_getavoid extends RecursiveTask<Integer> {
		int p1,p2,q1,q2;
		int n1, n2;
		boolean[] isFinal1;
		boolean[] isFinal2;
		...
		par_JumpingBLAFair_getavoid(boolean[][] Y, int n1, int n2, int p1, int p2, int q1, int q2,boolean[] isFinal1, boolean[] isFinal2, int n_symbols, int[][][] post1, int[][] post_len1, int[][][] post2, int[][] post_len2, int[][] jump, int[] jump_len, int[][] acc_jump, int[] acc_jump_len, boolean[][] W, boolean[][] X, int la){
			this.p1=p1;
			this.p2=p2;
			this.q1=q1;
			this.q2=q2;
			this.n1 = n1;
			this.n2 = n2;
			this.isFinal1 = isFinal1;
			this.isFinal2 = isFinal2;
			this.n_symbols = n_symbols;
			this.post1 = post1;
			this.post_len1 = post_len1;
			this.post2 = post2;
		...
		protected Integer compute(){
			if ((p2-p1 <= threshold_jumping_getavoid(n1)) || (q2-q1 <= threshold_jumping_getavoid(n2))) {
				return single_par_JumpingBLAFair_getavoid(Y,n1,n2,p1,p2,q1,q2,isFinal1,isFinal2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,X,la);
			}
			else{
				par_JumpingBLAFair_getavoid t1 = new par_JumpingBLAFair_getavoid(Y,n1,n2,p1,p1+(p2-p1)/2,q1,q1+(q2-q1)/2,isFinal1,isFinal2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,X,la);
				par_JumpingBLAFair_getavoid t2 = new par_JumpingBLAFair_getavoid(Y,n1,n2,p1,p1+(p2-p1)/2,q1+(q2-q1)/2,q2,isFinal1,isFinal2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,X,la);
				par_JumpingBLAFair_getavoid t3 = new par_JumpingBLAFair_getavoid(Y,n1,n2,p1+(p2-p1)/2,p2,q1,q1+(q2-q1)/2,isFinal1,isFinal2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,X,la);
				par_JumpingBLAFair_getavoid t4 = new par_JumpingBLAFair_getavoid(Y,n1,n2,p1+(p2-p1)/2,p2,q1+(q2-q1)/2,q2,isFinal1,isFinal2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,X,la);
	...
	private int single_par_JumpingBLAFair_getavoid(boolean[][] Y, int n1, int n2, int p1, int p2, int q1, int q2,boolean[] isFinal1, boolean[] isFinal2, int n_symbols, int[][][] post1, int[][] post_len1, int[][][] post2, int[][] post_len2, int[][] jump, int[] jump_len, int[][] acc_jump, int[] acc_jump_len, boolean[][] W, boolean[][] X, int la)
	{
		//TODO:
		boolean changed=false;
		int[] attack = new int[2*la+1];
		...
		for(int p=p1; p<p2; p++) {
			for (int q = q1; q < q2; q++) {
				...
				attack[0] = p;
				if (JumpingBLAFair_getavoid_attack(q, isFinal1, isFinal2, n_symbols, post1, post_len1, post2, post_len2, jump, jump_len, acc_jump, acc_jump_len, W, X, Y, la, attack, 0)) {
					Y[p][q] = true;
					changed = true;
				}
			}
...
// Start with X (i.e., avoid) as true and refine downward
		...
		while(changed_x){
			...
			while(changed_y){
				...
				changed_y=(forkJoinPool.invoke(new par_JumpingBLAFair_getavoid(Y,n1,n2,0,n1,0,n2,isFinal1,isFinal2,n_symbols,post1,post_len1,post2,post_len2,jump,jump_len,acc_jump,acc_jump_len,W,X,la)) >0);
...
//			changed=false;
			...
			changed=(forkJoinPool.invoke(new par_BLAFair_refine(0,n_states,0,n_states,isFinal,n_states,n_symbols,post,post_len,W,la)) >0);
	...
	//NOTICE:
	class par_BLAFair_refine extends RecursiveTask<Integer> {
		int p1,p2,q1,q2;
		boolean[] isFinal;
		int n_states;
		int n_symbols;
	...
	par_BLAFair_refine(int p1, int p2, int q1, int q2, boolean[] isFinal, int n_states, int n_symbols, int[][][] post, int[][] post_len, boolean[][] W, int la){
		this.p1=p1;
		this.p2=p2;
		this.q1=q1;
		this.q2=q2;
	...
	protected Integer compute(){
		if ((p2-p1 <= threshold_forkjoin_Fair_refine(n_states)) || (q2-q1 <= threshold_forkjoin_Fair_refine(n_states))) {
			return single_BLAFair_refine_W(p1,p2,q1,q2,n_symbols,post,post_len,W,la);
		}
		else{
			par_BLAFair_refine t1 = new par_BLAFair_refine(p1,p1+(p2-p1)/2,q1,q1+(q2-q1)/2,isFinal,n_states,n_symbols,post,post_len,W,la);
			par_BLAFair_refine t2 = new par_BLAFair_refine(p1,p1+(p2-p1)/2,q1+(q2-q1)/2,q2,isFinal,n_states,n_symbols,post,post_len,W,la);
			par_BLAFair_refine t3 = new par_BLAFair_refine(p1+(p2-p1)/2,p2,q1,q1+(q2-q1)/2,isFinal,n_states,n_symbols,post,post_len,W,la);
			par_BLAFair_refine t4 = new par_BLAFair_refine(p1+(p2-p1)/2,p2,q1+(q2-q1)/2,q2,isFinal,n_states,n_symbols,post,post_len,W,la);
			t2.fork();
			t3.fork();
			t4.fork();
			int r1=t1.compute();
	...
	private int single_BLAFair_refine_W(int p1, int p2, int q1, int q2, int n_symbols, int[][][] post, int[][] post_len, boolean[][] W, int la)
	{
		//TODO:
		int[] attack = new int[2*la+1];
		boolean changed=false;
...
//
	private boolean BLAFair_refine_W(int nstates, int n_symbols, int[][][] post, int[][] post_len, boolean[][] W, int la)
	{
		int[] attack = new int[2*la+1];
		boolean changed=false;
	...
	//NOTICE:
	class par_BLAFair_getavoid_check extends RecursiveTask<Integer> {
		int p1,p2,q1,q2;
		boolean[] isFinal;
		int n_states;
		int n_symbols;
		...
		par_BLAFair_getavoid_check(boolean[][] X, boolean[][] Y, int p1, int p2, int q1, int q2, boolean[] isFinal, int n_states, int n_symbols, int[][][] post, int[][] post_len, boolean[][] W, int la){
			this.p1=p1;
			this.p2=p2;
			this.q1=q1;
			this.q2=q2;
		...
		protected Integer compute(){
			if ((p2-p1 <= threshold_forkjoin_getavoid(n_states)) || (q2-q1 <= threshold_forkjoin_getavoid(n_states))) {
				return single_BLAFair_getavoid_check(X, Y,isFinal, p1,p2,q1,q2,n_symbols,post,post_len,W,la);
			}
			else{
				par_BLAFair_getavoid_check t1 = new par_BLAFair_getavoid_check(X,Y,p1,p1+(p2-p1)/2,q1,q1+(q2-q1)/2,isFinal,n_states,n_symbols,post,post_len,W,la);
				par_BLAFair_getavoid_check t2 = new par_BLAFair_getavoid_check(X,Y,p1,p1+(p2-p1)/2,q1+(q2-q1)/2,q2,isFinal,n_states,n_symbols,post,post_len,W,la);
				par_BLAFair_getavoid_check t3 = new par_BLAFair_getavoid_check(X,Y,p1+(p2-p1)/2,p2,q1,q1+(q2-q1)/2,isFinal,n_states,n_symbols,post,post_len,W,la);
				par_BLAFair_getavoid_check t4 = new par_BLAFair_getavoid_check(X,Y,p1+(p2-p1)/2,p2,q1+(q2-q1)/2,q2,isFinal,n_states,n_symbols,post,post_len,W,la);
				t2.fork();
				t3.fork();
				t4.fork();
				int r1=t1.compute();
	...
	private int single_BLAFair_getavoid_check(boolean[][] X, boolean[][] Y, boolean[] isFinal, int p1, int p2, int q1, int q2, int n_symbols, int[][][] post, int[][] post_len, boolean[][] W, int la)
	{
		//TODO:
		int[] attack = new int[2*la+1];
		boolean changed_y=false;
		for(int p=p1; p<p2; p++)
			for(int q=q1; q<q2; q++){
				if(Y[p][q]) continue; // If Y true then stay true
...
// Start with X (i.e., avoid) as true and refine downward
		...
		while (changed_x) {
			...
			while (changed_y) {
				//changed_y = false;
				changed_y = (forkJoinPool.invoke(new par_BLAFair_getavoid_check(X, Y, 0, n, 0, n, isFinal, n, n_symbols, post, post_len, W, la)) > 0);
...
// Unlike pre_refine, it is NOT correct to use for delayed/fair sim, but only for direct sim.
	...
	/**
	 * Compute direct simulation relation on a Buchi automaton, by naive fixpoint iteration
...
// ------------------------------------------------------------------------------------------------------------
	...
	/**
	 * Compute backward simulation relation on a Buchi automaton, by naive fixpoint iteration (and bitsets)
	 * @param omega1: a Buchi automaton
	 *
	 * @return direct simulation
	 */
